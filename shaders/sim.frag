...
const float COLLISION_PROB=.08;
const float ATOM_RADIUS=2.;
...
void main(){
  ...
  if((col+1)%7==0){
    ...
    float adaptedRadius=ATOM_RADIUS*(COLLISION_PROB*(1./speed));
  ...
  if(hitID>0.){
    outColor=vec4(0.,0.,0.,hitID);// Alpha kantaa ID:n vielä kerran CPU:lle
  }else{
